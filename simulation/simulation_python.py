...
# This python file will show that we how to apply ABESS-DAG and 
# NOTEARS on the simulated data. 
...
# Import data, we only use p=30 as a demonstration.

for st in ["ER2","ER5","BA2","BA5"]:
    for y in [30,100,300]:
        result1 = pd.DataFrame(data=None, columns=["Time","TPR","FDR","SHD"])
        result2 = pd.DataFrame(data=None, columns=["Time","TPR","FDR","SHD"])
        for k in range(100):
            pth = os.getcwd()+r"\data\{st}\p30\n{y}\{st}_p30_n{y}_{k}.csv".format(st=st,y=y,k=k)
            data = pd.read_csv(pth)
            data = data.iloc[: , 1:]
            pth_info = os.getcwd()+r"\data\{st}\graph\{st}_p30_{k}.csv".format(st=st,k=k)
            ...
            pth_out = os.getcwd()+r"\data\{st}\p30\n{y}".format(st=st,y=y)
...
for st in ["ER2","ER5","BA2","BA5"]:
    for y in [50,200,500]:
        ...
        result2 = pd.DataFrame(data=None, columns=["Time","TPR","FDR","SHD"])
        for k in range(100):
            pth = os.getcwd()+r"\data\{st}\p100\n{y}\{st}_p100_n{y}_{k}.csv".format(st=st,y=y,k=k)
            data = pd.read_csv(pth)
            data = data.iloc[: , 1:]
            pth_info = os.getcwd()+r"\data\{st}\graph\{st}_p100_{k}.csv".format(st=st,k=k)
            ginfo = pd.read_csv(pth_info)
            start = time.perf_counter()
            w = abessdag(data)
            end = time.perf_counter()
            ...
            pth_out = os.getcwd()+r"\data\{st}\p100\n{y}".format(st=st,y=y)
            result1.to_csv(pth_out+r"\Result_abessdag.csv")
            print(st,y,k)
...
for st in ["ER2","ER5","BA2","BA5"]:
    for y in [100,500,1000]:
        ...
        result2 = pd.DataFrame(data=None, columns=["Time","TPR","FDR","SHD"])
        for k in range(100):
            pth = os.getcwd()+r"\data\{st}\p500\n{y}\{st}_p500_n{y}_{k}.csv".format(st=st,y=y,k=k)
            data = pd.read_csv(pth)
            data = data.iloc[: , 1:]
            pth_info = os.getcwd()+r"\data\{st}\graph\{st}_p500_{k}.csv".format(st=st,k=k)
            ginfo = pd.read_csv(pth_info)
            start = time.perf_counter()
            w = abessdag(data)
            end = time.perf_counter()
            ...
            pth_out = os.getcwd()+r"\data\{st}\p500\n{y}".format(st=st,y=y)
            result1.to_csv(pth_out+r"\Result_abessdag.csv")
            print(st,y,k)
